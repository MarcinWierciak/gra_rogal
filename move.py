...
def go_player(board, x, y, sign, monsters, items, player_stats, chests): # porusza postacią gracza
    direction = getch()
    move_status = {"x_coord":0,"y_coord":0,"action":"", "found_object":""}
    new_x = x
    new_y = y
    added_item = ""
    if direction == "s":
    ...
    if new_xy in monsters:
        ...
        move_status.update({"x_coord":x,"y_coord":y,"action":"fight","found_object":new_xy})
        return move_status
    if new_xy in chests:
        print("You found chest. It's locked.")
        move_status.update({"x_coord":x,"y_coord":y,"action":"chest", "found_object":new_xy})
        return move_status
    move_status.update({"x_coord":x,"y_coord":y})
    return move_status
